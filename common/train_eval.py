...
def train(main_script_path, func_train_one_batch, param_dict, savev_distance_matrix=False, path=None):
    ...
    pos_gen_optimizer = get_optimizer(model_pos_gen)
    neg_gen_optimizer = get_optimizer(model_neg_gen)
    ...
    for epoch in range(p.num_epochs):
        ...
        for batch in tqdm(train_loader, desc='# {}'.format(epoch)):
            loss_gen, loss_dis = func_train_one_batch(device, model, 
                model_pos_gen, model_neg_gen, model_dis,
